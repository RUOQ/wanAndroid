 ...
 * 继承于这个特定的 @[Fragment], 然后再按照 [com.jess.arms.base.BaseFragment] 的格式, 将代码复制过去,
 * 记住一定要实现[IFragment]
 *
...
abstract class BaseFragment<P : IPresenter> : SupportFragment(), IFragment, FragmentLifecycleable,
    ...
    override fun provideCache(): Cache<String, Any> {
        if (mCache == null) {
            mCache = ArmsUtils.obtainAppComponentFromContext(activity).cacheFactory().build(
                CacheType.ACTIVITY_CACHE)
        }
